...
const terminalCommands = {
  skills: () =>
    "Flutter, Dart, Firebase, Bloc, Provider, Riverpod, Clean Architecture, MVVM, OOP, SOLID Principles",
  ...
  help: () =>
    "Available commands: skills, projects, contact, joke, clear, exit",
  exit: () => {
    hideTerminal();
    return "Goodbye! 👋";
  },
...
document
  .getElementById("terminalClose")
  ?.addEventListener("click", hideTerminal);
...
async function loadDataWithData(data) {
  ...
  phoneA.textContent = data.personal_info.contact.phone;
  emailA.href = emailHref;
  emailA.textContent = data.personal_info.contact.email;
  ...
  if (linkedinLink) {
    ...
    linkedinA.textContent = "Amira Shinnawi";
  }
  ...
  (data.projects || []).forEach((p, idx) => {
    const card = el(`
      <article class="project" data-aos="fade-up" data-aos-delay="${
        (idx % 3) * 100
