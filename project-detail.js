...
function populateProjectData(project) {
  ...
  document.getElementById(
    "pageTitle"
  ).textContent = `${project.name} | Amira Shinnawi`;
  document.title = `${project.name} | Amira Shinnawi`;
        ...
        galleryItem.innerHTML = `
          ...
          <div class="gallery-overlay">
            <div class="gallery-type">${
              media.type === "gif" ? "Demo" : "Screenshot"
            }</div>
          </div>
        `;
...
function initializeGallery() {
  ...
  const images = Array.from(galleryItems).filter(
    (item) => item.querySelector("img") && !item.querySelector("video")
  );
  ...
  imageModal.addEventListener("click", (e) => {
    if (
      e.target === imageModal ||
      e.target.classList.contains("image-modal__backdrop")
    ) {
      closeImageModal();
    }
  });
